...
char *g_out = nullptr;
unsigned char *g_cur_out = nullptr;
...
__global__ void sha256_kernel(char* out_input_string_nonce, unsigned char* out_found_hash, unsigned char* out_hash, int *out_found, const char* in_input_string, size_t in_input_string_size, uint8_t difficulty, uint64_t nonce_offset) {
...
int main() {
	...
	cudaMallocManaged(&g_cur_out, 32);
	...
	*g_found = 0;
	...
	while (!*g_found) {
		sha256_kernel << < NUMBLOCKS, BLOCK_SIZE, dynamic_shared_size >> > (g_out, g_hash_out,g_cur_out, g_found, d_in, input_size, difficulty, nonce);
